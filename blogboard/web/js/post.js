...
async function loadPost() {
    ...
    // Fetch and render markdown — try local first, then R2
    try {
        let mdText = null;
        const localUrl = blog.file;
        const r2Url = `${R2_PUBLIC_URL}/${blog.file}`;
        ...
        if (!mdText) throw new Error('All fetch attempts failed');
